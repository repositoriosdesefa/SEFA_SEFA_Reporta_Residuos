...
output: 
  word_document:
    reference_docx: "plantilla_RR_sc.docx"
---
...
TABLA_ALERTAS <- REPORTE_A_D %>%
  ...
  select(ID_GEN, NOMBRE_LISTA_EFA, FECHA_TOMA,
         HORA_FOTO,`DISTRITO`,DIAS_TRANSCURRIDOS,`LONGITUD`,`LATITUD`)%>%
  arrange(DISTRITO,FECHA_TOMA)

Cabeceras <- c("Codigo","Municipalidad","Fecha_de_alerta","Hora_alerta","Dias",
...
valores_separados<- function(Cod, Muni, Fecha, Hora, Dias,
  ...
  for (i in 1:N_ALERTAS[1]){
    b= paste0(b,separa(Cod[i], Muni[i], Fecha[i], Hora[i], Dias[i],
                       Dist[i], Long[i], Lat[i]))
  }
...
crea_tabla <- function(Cod, Muni, Fecha, Hora, Dias, Dist, Long, Lat){
  ...
  tabla <- paste0(inicio, valores_separados(Cod, Muni, Fecha, Hora, Dias, Dist, Long, Lat))
...
tabla_alerta <- crea_tabla(TABLA_ALERTAS$Codigo,
                           ...
                           TABLA_ALERTAS$Longitud,
                           TABLA_ALERTAS$Latitud)
...
Al respecto, les recordamos que 'Reporta Residuos' cuenta con un 'Mapa de alertas en tiempo real' al que pueden acceder desde el aplicativo móvil o el portal <https://publico.oefa.gob.pe/Portalpifa/mapasEspecializados.do>, en donde se pueden observar todas las alertas de acumulación de basura enviadas por la ciudadanía cuyo estado se diferencia por colores[^2].
